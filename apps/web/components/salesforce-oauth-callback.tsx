...
export function SalesforceOAuthCallback() {
  ...
  const [message, setMessage] = useState(
    completeResponse
      ? "Verifying Salesforce authorisation…"
      : "This Salesforce authorisation response is incomplete. Start the connection again.",
  );
  ...
  useEffect(() => {
    ...
    apiRequest<OrganisationConnection>(
      ...
      .then((connection) => {
        setState("connected");
        setMessage(
          `Salesforce organisation ${connection.externalAccountName ?? connection.externalTenantId ?? ""} is connected in read-only sync mode.`,
        );
      })
      .catch((reason: unknown) => {
        ...
        setMessage(
          reason instanceof Error
            ? reason.message
            : "Salesforce could not be connected.",
        );
      });
