...
export function GoogleOAuthCallback() {
  ...
  const [message, setMessage] = useState(
    completeResponse
      ? "Verifying Google authorisation…"
      : "This Google authorisation response is incomplete. Start the connection again.",
  );
  ...
  useEffect(() => {
    ...
    apiRequest<OrganisationConnection>(
      ...
      .then((connection) => {
        setState("connected");
        setMessage(
          `${connection.externalAccountEmail ?? "Your Google Workspace account"} is connected.`,
        );
      })
