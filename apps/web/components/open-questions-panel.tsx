...
export function OpenQuestionsPanel({ meetingId }: { meetingId: string }) {
  const [result, setResult] = useState<OpenQuestionsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const next = await apiRequest<OpenQuestionsResponse>(
          `/api/v1/meetings/${meetingId}/intelligence/open-questions`,
        ...
        setError(
          requestError instanceof Error
            ...
            : "Open Questions status could not be loaded.",
        );
        setLoading(false);
      }
    }
  ...
  async function generate() {
    ...
    setError(null);
    try {
      await apiRequest<OpenQuestionsRequestResponse>(
        `/api/v1/meetings/${meetingId}/intelligence/open-questions`,
        { method: "POST" },
      );
      setResult((current) =>
      ...
      setError(
        requestError instanceof Error
          ? requestError.message
          : "Open Questions generation could not be requested.",
      );
    } finally {
      setGenerating(false);
    }
  ...
  if (error) {
    return (
      <div role="alert" className="form-card border-rose-200 bg-rose-50">
        <h2 className="form-legend text-rose-950">Open Questions</h2>
  ...
  if (!result || result.state === "empty") {
    return (
      <div className="form-card">
        <h2 className="form-legend">Open Questions</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
          {result?.unavailableReason ??
            "Identify questions that remain unresolved after the entire transcript."}
        </p>
        {result?.generationAvailable ? (
          <button
            ...
            onClick={generate}
          >
            {generating ? "Requesting…" : "Generate Open Questions"}
          </button>
        ) : null}
      </div>
    );
    ...
    return (
      <div className="form-card" aria-live="polite">
        ...
        <p role="status" className="mt-3 text-sm text-slate-700">
          {result.state === "queued"
            ? "Open Questions generation is queued…"
  ...
  if (result.state === "failed" || result.state === "cancelled") {
    return (
      <div className="form-card border-amber-200 bg-amber-50">
        <h2 className="form-legend text-amber-950">Open Questions</h2>
  ...
  const content = result.openQuestions;
  if (!content) {
    return (
      <div role="alert" className="form-card border-rose-200 bg-rose-50">
        Open Questions content is unavailable. Try refreshing the page.
      </div>
    );
  }

  return (
    <article className="form-card" aria-labelledby="open-questions-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <h2 id="open-questions-heading" className="form-legend">
          Open Questions
        </h2>
          ...
          {content.openQuestions.map((item, index) => (
            ...
            >
              ...
              <dl className="mt-3 grid gap-x-6 gap-y-3 sm:grid-cols-3">
                <QuestionDetail label="Importance">
                  {humanise(item.importance)}
                </QuestionDetail>
                {item.owner ? (
                  <QuestionDetail label="Owner">{item.owner}</QuestionDetail>
                ) : null}
                <QuestionDetail label="Confidence">
