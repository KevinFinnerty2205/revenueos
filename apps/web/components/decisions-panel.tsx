...
export function DecisionsPanel({ meetingId }: { meetingId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const next = await apiRequest<DecisionsResponse>(
          `/api/v1/meetings/${meetingId}/intelligence/decisions`,
        ...
        setError(
          requestError instanceof Error
            ...
            : "Decisions status could not be loaded.",
        );
        setLoading(false);
      }
    }
  ...
  async function generate() {
    ...
    setError(null);
    try {
      await apiRequest<DecisionsRequestResponse>(
        `/api/v1/meetings/${meetingId}/intelligence/decisions`,
        { method: "POST" },
      );
      setResult((current) =>
      ...
      setError(
        requestError instanceof Error
          ? requestError.message
          : "Decisions generation could not be requested.",
      );
    } finally {
      setGenerating(false);
    }
  ...
  if (error) {
    return (
      <div role="alert" className="form-card border-rose-200 bg-rose-50">
        <h2 className="form-legend text-rose-950">Decisions</h2>
  ...
  if (!result || result.state === "empty") {
    return (
      <div className="form-card">
        <h2 className="form-legend">Decisions</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
          {result?.unavailableReason ??
            "Identify decisions grounded in this meeting transcript."}
        </p>
        {result?.generationAvailable ? (
          <button
            ...
            onClick={generate}
          >
            {generating ? "Requesting…" : "Generate Decisions"}
          </button>
        ) : null}
      </div>
    );
    ...
    return (
      <div className="form-card" aria-live="polite">
        ...
        <p role="status" className="mt-3 text-sm text-slate-700">
          {result.state === "queued"
            ? "Decisions generation is queued…"
  ...
  if (result.state === "failed" || result.state === "cancelled") {
    return (
      <div className="form-card border-amber-200 bg-amber-50">
        <h2 className="form-legend text-amber-950">Decisions</h2>
        <p role="alert" className="mt-3 text-sm text-amber-900">
          {result.safeMessage ?? "Decisions generation could not be completed."}
        </p>
        {result.generationAvailable ? (
          <button
            type="button"
  ...
  const content = result.decisions;
  if (!content) {
    return (
      <div role="alert" className="form-card border-rose-200 bg-rose-50">
        Decisions content is unavailable. Try refreshing the page.
      </div>
    );
  }

  return (
    <article className="form-card" aria-labelledby="decisions-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <h2 id="decisions-heading" className="form-legend">
          Decisions
        </h2>
