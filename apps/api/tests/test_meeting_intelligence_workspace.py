...
@pytest.mark.parametrize(
    ...
    (
        ...
        (True, 10, 0, 0, 0, False, "completed"),
        (True, 10, 0, 0, 0, True, "completed_with_empty_results"),
    ),
)
...
def test_aggregate_read_is_product_safe_and_unavailable_without_transcript(
    ...
    assert body["nextBestAction"]["state"] == "unavailable"
...
def test_generation_orchestrates_missing_work_and_composer_dependencies(
    ...
    assert first.json()["nextBestAction"]["state"] == "unavailable"
    ...
    assert prerequisites_ready.json()["nextBestAction"]["state"] == "not_generated"

    composer = client.post(f"{base}/generate")
    assert composer.status_code == 202
    assert composer.json()["createdCapabilities"] == ["next_best_action", "follow_up_email"]
    assert composer.json()["followUpEmail"]["state"] == "queued"
    assert composer.json()["nextBestAction"]["state"] == "queued"
    assert _count_jobs() == 10
    ...
    assert body["nextBestAction"]["content"]["recommendedActions"]
