...
def test_health_aliases_are_safe_and_migration_head_is_current(
    ...
    assert EXPECTED_MIGRATION_HEAD == "0061_manual_paid_credit_grant"
...
def test_export_is_deterministic_tenant_scoped_and_excludes_internal_fields(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        ...
        assert payload["exportVersion"] == 37
        ...
        assert payload["crmEntityMappings"] == []
        assert payload["crmFieldMappings"] == []
        assert payload["crmStageMappings"] == []
        assert "crmSyncCursors" not in payload
        assert payload["actionExecutions"] == []
        assert payload["actionExecutionAttempts"] == []
        assert payload["integrationAuditEvents"] == []
        assert payload["createTemplates"] == []
        assert payload["createTemplateVersions"] == []
        assert payload["createTemplateSlides"] == []
        assert payload["createApprovedContentItems"] == []
        assert payload["createPresentations"] == []
        assert payload["createPresentationVersions"] == []
        assert payload["createValueModels"] == []
