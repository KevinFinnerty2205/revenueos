...
logger = logging.getLogger("revenueos.google_sync_worker")
DISCOVERY_LIMIT = 1000


class GoogleSyncWorkerService:
    """Runs bounded Gmail history and Calendar token sync in the durable worker."""
    ...
    async def run_once(self) -> bool:
        if not (self._settings.feature_integrations_enabled and self._settings.feature_google_workspace_enabled):
            return False
        processed = False
        due_before = datetime.now(UTC) - timedelta(seconds=self._settings.google_sync_interval_seconds)
            ...
            if session.get_bind().dialect.name == "postgresql":
                values = await session.scalars(
                    text(
                        ...
                        FROM public.revenueos_mailbox_sync_eligible_organisations(
                            :provider_key,
                            :due_before,
                            :result_limit
                        )"""
    ...
    async def _sync_one(self, organisation_id: UUID, due_before: datetime) -> bool:
        async with self._session_factory() as session:
            ...
            try:
                await GoogleSyncService(session, tenant, self._settings).sync(connection.id)
            except PublicAPIError as exc:
                logger.warning(
                    "google_sync_worker_failed",
                    extra={
