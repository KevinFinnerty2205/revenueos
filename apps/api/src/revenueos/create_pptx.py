...
CREATE_PPTX_PROFILE_VERSION: Final = 1
MAX_PPTX_COMPRESSION_RATIO: Final = 200
MAX_PPTX_AGGREGATE_COMPRESSION_RATIO: Final = 100
MAX_PPTX_PATH_CHARACTERS: Final = 255
MAX_PPTX_XML_DEPTH: Final = 128
MAX_PPTX_XML_ELEMENTS: Final = 100_000
MAX_PPTX_RELATIONSHIPS_PER_PART: Final = 1_000
MAX_PPTX_RELATIONSHIPS: Final = 10_000
...
class GeneratedOutputValidationError(UnsafePptxError):
    code = "generated_validation_failed"
...
@dataclass(frozen=True)
class OutputSlideExpectation:
    title: str
    body_blocks: tuple[str, ...]
    exact_text_blocks: tuple[str, ...]
    required: bool
    replacement_texts: tuple[str, ...] = ()
    replacement_shape_texts: tuple[tuple[int, str], ...] = ()
...
class BoundedPptxProcessor:
        ...
        try:
            ...
            for render_slide, slide in zip(slides, selected, strict=True):
                ...
                for shape in slide.shapes:
                    ...
                    if not shape.has_text_frame:
                        raise UnsafePptxError
                    source_paragraph = shape.text_frame.paragraphs[0]
                    source_run = source_paragraph.runs[0] if source_paragraph.runs else None
                    font_name = (source_run.font.name if source_run is not None else None) or source_paragraph.font.name
                    font_size = (source_run.font.size if source_run is not None else None) or source_paragraph.font.size
                    font_bold = source_run.font.bold if source_run is not None else None
                    if font_bold is None:
                        font_bold = source_paragraph.font.bold
                    font_italic = source_run.font.italic if source_run is not None else None
                    if font_italic is None:
                        font_italic = source_paragraph.font.italic
                    source_color = (
                        source_run.font.color
                        if source_run is not None and source_run.font.color.type is not None
                        else source_paragraph.font.color
                    )
                    rgb = source_color.rgb if source_color.type == MSO_COLOR_TYPE.RGB else None
                    theme_color = source_color.theme_color if source_color.type == MSO_COLOR_TYPE.SCHEME else None
                    alignment = source_paragraph.alignment
                    level = source_paragraph.level
                    ...
                    for paragraph in shape.text_frame.paragraphs:
                        ...
                        if rgb is not None:
                            ...
                            paragraph.font.color.theme_color = theme_color
    ...
    def validate_output(
        self,
        content: bytes,
        expectations: tuple[OutputSlideExpectation, ...],
        *,
        forbidden_values: tuple[str, ...] = (),
    ) -> ValidatedOutput:
        """Reparse generated bytes and prove the structured review contract."""
        entries = self._preflight(content)
        if any(
            path.startswith(("ppt/notesSlides/", "ppt/notesMasters/", "ppt/comments/", "customXml/"))
            or path in {"ppt/commentAuthors.xml", "docProps/custom.xml", "docProps/thumbnail.jpeg"}
            for path in entries
        ):
        ...
        try:
            ...
            raise MalformedPptxError from exc
        if len(presentation.slides) != len(expectations):
            raise GeneratedOutputValidationError
        warnings: set[str] = set()
        package_text = "\n".join(
            data.decode("utf-8", errors="ignore") for path, data in entries.items() if path.endswith((".xml", ".rels"))
        )
        for forbidden in forbidden_values:
            if forbidden and forbidden in package_text:
                raise GeneratedOutputValidationError
        for slide, expectation in zip(presentation.slides, expectations, strict=True):
            visible = tuple(
                text for shape in slide.shapes if shape.has_text_frame and (text := _normalise_text(shape.text))
            )
            visible_by_shape = {
                shape.shape_id: _normalise_text(shape.text) for shape in slide.shapes if shape.has_text_frame
            }
            combined = "\n".join(visible)
            if expectation.title and not _expected_text_present(expectation.title, visible, combined):
                raise GeneratedOutputValidationError
            for expected in expectation.body_blocks:
                if expected and not _expected_text_present(expected, visible, combined):
                    raise GeneratedOutputValidationError
            for exact in expectation.exact_text_blocks:
                if exact and exact not in visible:
                    raise GeneratedOutputValidationError
            for replacement in expectation.replacement_texts:
                if replacement and not _expected_text_present(replacement, visible, combined):
                    raise GeneratedOutputValidationError
            for shape_id, replacement in expectation.replacement_shape_texts:
                if visible_by_shape.get(shape_id) != _normalise_text(replacement):
                    raise GeneratedOutputValidationError
            if expectation.required and not visible:
                raise GeneratedOutputValidationError
            if _slide_needs_overflow_review(slide):
                warnings.add("text_overflow_review_recommended")
        return ValidatedOutput(slide_count=len(presentation.slides), warning_codes=tuple(sorted(warnings)))

    def _preflight(self, content: bytes) -> dict[str, bytes]:
        if not content or len(content) > self.limits.max_bytes:
            raise PptxLimitError
        _validate_zip_envelope(content)
            ...
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                ...
                for info in infos:
                    ...
                    if info.compress_size == 0 and info.file_size > 0:
                        raise PptxLimitError
                    if info.file_size > 1_000_000 and info.file_size / info.compress_size > MAX_PPTX_COMPRESSION_RATIO:
                        raise PptxLimitError
                    ...
                    if name.startswith("ppt/media/"):
                        ...
                        if media_count > self.limits.max_media_assets or len(data) > self.limits.max_media_bytes:
                            raise PptxLimitError
                        _validate_media(data, PurePosixPath(name).suffix)
                    if name.endswith((".xml", ".rels")):
                        if len(data) > self.limits.max_xml_bytes:
                            raise PptxLimitError
                        _parse_safe_xml(data)
                    entries[name] = data
                if compressed == 0 or expanded / compressed > MAX_PPTX_AGGREGATE_COMPRESSION_RATIO:
                    raise PptxLimitError
        except PptxProcessingError:
            raise
        ...
        for name, data in entries.items():
            ...
            if len(relationships) > MAX_PPTX_RELATIONSHIPS_PER_PART:
                raise PptxLimitError
            relationship_count += len(relationships)
            if relationship_count > MAX_PPTX_RELATIONSHIPS:
                raise PptxLimitError
            for relationship in relationships:
                ...
                if not target or _relationship_target(name, target) not in entries:
                    raise MalformedPptxError
        return entries
...
def _safe_zip_name(value: str) -> str:
    if (
        not value
        or len(value) > MAX_PPTX_PATH_CHARACTERS
        or "\x00" in value
        or "\\" in value
        or value.startswith(("/", "."))
    ...
    if any(part in {"", ".", ".."} or len(part) > 200 for part in path.parts):
        raise UnsafePptxError
    normalised = str(path)
...
def _validate_zip_envelope(content: bytes) -> None:
    if not content.startswith(b"PK\x03\x04"):
        raise MalformedPptxError
    eocd_offset = content.rfind(b"PK\x05\x06", max(0, len(content) - 65_557))
    if eocd_offset < 0 or eocd_offset + 22 > len(content):
        raise MalformedPptxError
    comment_size = int.from_bytes(content[eocd_offset + 20 : eocd_offset + 22], "little")
    if eocd_offset + 22 + comment_size != len(content):
        raise MalformedPptxError
    if b"PK\x06\x06" in content or b"PK\x06\x07" in content:
        raise UnsupportedPptxError


def _relationship_target(relationship_path: str, raw_target: str) -> str:
    parsed = urllib.parse.urlsplit(raw_target)
    if parsed.scheme or parsed.netloc or parsed.query or parsed.fragment or "\x00" in raw_target or "\\" in raw_target:
        raise UnsafePptxError
    ...
    if relationship_path == "_rels/.rels":
        ...
        marker = "/_rels/"
        if marker not in relationship_path or not relationship_path.endswith(".rels"):
            raise MalformedPptxError
        prefix, relation_name = relationship_path.split(marker, 1)
        source_part = f"{prefix}/{relation_name.removesuffix('.rels')}"
        source_parent = posixpath.dirname(source_part)
    if decoded.startswith("/"):
        normalised = posixpath.normpath(decoded.lstrip("/"))
    else:
        normalised = posixpath.normpath(posixpath.join(source_parent, decoded))
    if normalised in {"", ".", ".."} or normalised.startswith("../"):
        raise UnsafePptxError
    return _safe_zip_name(normalised)
        ...
        for offset in range(0, len(content), 65_536):
            ...
            for raw_event in parser.read_events():
                ...
                if event == "start":
                    count += 1
                    depth += 1
                    if depth > MAX_PPTX_XML_DEPTH or count > MAX_PPTX_XML_ELEMENTS:
                        raise PptxLimitError
...
def _validate_media(content: bytes, extension: str | None = None) -> None:
    detected = next((kind for signature, kind in _ALLOWED_MEDIA_SIGNATURES if content.startswith(signature)), None)
    allowed_extensions = {
        "png": {".png"},
        "jpeg": {".jpg", ".jpeg"},
        "gif": {".gif"},
    }
    if detected is None or (extension is not None and extension.casefold() not in allowed_extensions[detected]):
        raise UnsupportedPptxError
...
def _expected_text_present(expected: str, visible: tuple[str, ...], combined: str) -> bool:
    normalised = _normalise_text(expected)
    return normalised in visible or normalised in combined


def _slide_needs_overflow_review(slide: object) -> bool:
    for shape in getattr(slide, "shapes", ()):
        if not getattr(shape, "has_text_frame", False):
            continue
        text = _normalise_text(str(getattr(shape, "text", "")))
        width = int(getattr(shape, "width", 0) or 0)
        height = int(getattr(shape, "height", 0) or 0)
        if not text or width <= 0 or height <= 0:
            continue
        width_inches = width / 914_400
        height_inches = height / 914_400
        characters_per_line = max(8, math.floor(width_inches * 9))
        available_lines = max(1, math.floor(height_inches * 3.5))
        estimated_lines = sum(
            max(1, math.ceil(len(line) / characters_per_line)) for line in text.splitlines() or (text,)
        )
