"""Allow Stakeholder Intelligence AI jobs and artefacts.

Revision ID: 0015_stakeholders
Revises: 0014_objections
"""
...
revision: str = "0015_stakeholders"
down_revision: str | None = "0014_objections"
...
def _replace_type_constraints(*, include_stakeholders: bool) -> None:
    common_job_values = (
    ...
    job_values = (
        common_job_values + "'stakeholder_intelligence', 'follow_up_email')"
        if include_stakeholders
        else common_job_values + "'follow_up_email')"
    )
    artifact_values = (
        common_artifact_values + "'stakeholder_intelligence', 'follow_up_email')"
        if include_stakeholders
...
def upgrade() -> None:
    _replace_type_constraints(include_stakeholders=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'stakeholder_intelligence'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'stakeholder_intelligence'")
    _replace_type_constraints(include_stakeholders=False)
