"""Allow Next Best Action AI jobs and artefacts.

Revision ID: 0016_next_best_action
Revises: 0015_stakeholders
"""
...
revision: str = "0016_next_best_action"
down_revision: str | None = "0015_stakeholders"
...
def _restore_sqlite_artifact_immutability_triggers() -> None:
    ...
    op.execute(
        ...
        BEGIN
            SELECT RAISE(
                ABORT,
                'AI artefact supersession is immutable once set'
            );
        END
        """
    )
...
def _replace_type_constraints(*, include_next_best_action: bool) -> None:
    common_job_values = (
    ...
    job_values = (
        common_job_values + "'next_best_action', 'follow_up_email')"
        if include_next_best_action
        else common_job_values + "'follow_up_email')"
    )
    artifact_values = (
        common_artifact_values + "'next_best_action', 'follow_up_email')"
        if include_next_best_action
...
def upgrade() -> None:
    _replace_type_constraints(include_next_best_action=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'next_best_action'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'next_best_action'")
    _replace_type_constraints(include_next_best_action=False)
