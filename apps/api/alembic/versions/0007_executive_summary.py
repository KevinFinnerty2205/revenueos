"""Allow Executive Summary AI jobs and artefacts.

Revision ID: 0007_executive_summary
Revises: 0006_ai_worker_queue
"""
...
revision: str = "0007_executive_summary"
down_revision: str | None = "0006_ai_worker_queue"
...
def _replace_type_constraints(*, include_executive_summary: bool) -> None:
    job_values = (
        "job_type IN ('infrastructure_test', 'executive_summary')"
        if include_executive_summary
        else "job_type IN ('infrastructure_test')"
    )
    artifact_values = (
        "artifact_type IN ('infrastructure_test', 'executive_summary')"
        if include_executive_summary
        else "artifact_type IN ('infrastructure_test')"
    )
...
def upgrade() -> None:
    _replace_type_constraints(include_executive_summary=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'executive_summary'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'executive_summary'")
    _replace_type_constraints(include_executive_summary=False)
