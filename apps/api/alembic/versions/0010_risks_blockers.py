"""Allow Risks & Blockers AI jobs and artefacts.

Revision ID: 0010_risks_blockers
Revises: 0009_action_items
"""
...
revision: str = "0010_risks_blockers"
down_revision: str | None = "0009_action_items"
...
def _replace_type_constraints(*, include_risks_blockers: bool) -> None:
    job_values = (
        "job_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items', 'risks_blockers')"
        if include_risks_blockers
        else "job_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items')"
    )
    artifact_values = (
        "artifact_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items', 'risks_blockers')"
        if include_risks_blockers
        else "artifact_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items')"
    )
...
def upgrade() -> None:
    _replace_type_constraints(include_risks_blockers=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'risks_blockers'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'risks_blockers'")
    _replace_type_constraints(include_risks_blockers=False)
