"""Allow Open Questions AI jobs and artefacts.

Revision ID: 0011_open_questions
Revises: 0010_risks_blockers
"""
...
revision: str = "0011_open_questions"
down_revision: str | None = "0010_risks_blockers"
...
def _replace_type_constraints(*, include_open_questions: bool) -> None:
    job_values = (
        ...
        else "job_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items', 'risks_blockers')"
    )
    artifact_values = (
...
def upgrade() -> None:
    _replace_type_constraints(include_open_questions=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'open_questions'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'open_questions'")
    _replace_type_constraints(include_open_questions=False)
