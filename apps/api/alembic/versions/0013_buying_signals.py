"""Allow Buying Signals AI jobs and artefacts.

Revision ID: 0013_buying_signals
Revises: 0012_follow_up_email
"""
...
revision: str = "0013_buying_signals"
down_revision: str | None = "0012_follow_up_email"
...
def _replace_type_constraints(*, include_buying_signals: bool) -> None:
    job_values = (
        "job_type IN ('infrastructure_test', 'executive_summary', 'decisions', "
        "'action_items', 'risks_blockers', 'open_questions', 'buying_signals', 'follow_up_email')"
        if include_buying_signals
...
def upgrade() -> None:
    _replace_type_constraints(include_buying_signals=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'buying_signals'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'buying_signals'")
    _replace_type_constraints(include_buying_signals=False)
