"""Allow Action Items AI jobs and artefacts.

Revision ID: 0009_action_items
...
revision: str = "0009_action_items"
down_revision: str | None = "0008_decisions"
...
def _replace_type_constraints(*, include_action_items: bool) -> None:
    job_values = (
        "job_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items')"
        if include_action_items
        else "job_type IN ('infrastructure_test', 'executive_summary', 'decisions')"
    )
    artifact_values = (
        "artifact_type IN ('infrastructure_test', 'executive_summary', 'decisions', 'action_items')"
        if include_action_items
...
def upgrade() -> None:
    _replace_type_constraints(include_action_items=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'action_items'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'action_items'")
    _replace_type_constraints(include_action_items=False)
