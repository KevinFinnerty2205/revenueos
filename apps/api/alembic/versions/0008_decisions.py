"""Allow Decisions AI jobs and artefacts.

Revision ID: 0008_decisions
Revises: 0007_executive_summary
"""
...
revision: str = "0008_decisions"
down_revision: str | None = "0007_executive_summary"
...
def _replace_type_constraints(*, include_decisions: bool) -> None:
    job_values = (
        "job_type IN ('infrastructure_test', 'executive_summary', 'decisions')"
        if include_decisions
        else "job_type IN ('infrastructure_test', 'executive_summary')"
    )
    artifact_values = (
        "artifact_type IN ('infrastructure_test', 'executive_summary', 'decisions')"
        if include_decisions
        else "artifact_type IN ('infrastructure_test', 'executive_summary')"
    )
...
def upgrade() -> None:
    _replace_type_constraints(include_decisions=True)


def downgrade() -> None:
    op.execute("DELETE FROM ai_artifacts WHERE artifact_type = 'decisions'")
    op.execute("DELETE FROM ai_jobs WHERE job_type = 'decisions'")
    _replace_type_constraints(include_decisions=False)
