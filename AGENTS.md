...
Implement only the active task or sprint specification. Sprints 1–3 and WO-004A1/A2 are the current baseline: foundation; tenant-isolated CRUD for companies, contacts, opportunities and tasks; the tenant-isolated Meeting Domain with participants, deliberately supplied plain-text transcripts, soft deletion and audit metadata; tenant-isolated AI job/artefact models; and internal tenant-scoped AI repositories/services for idempotent infrastructure-test jobs, explicit lifecycle validation, strict artefact schema/versioning and metadata-only audits. The target [master product blueprint](docs/01-product/master-product-blueprint.md) and [roadmap](docs/06-roadmap/product-roadmap-to-beta.md) guide future sprint planning but do not authorise implementation. Workers, job claiming/locks, providers, prompts, AI API/UI access, real intelligence, recording, media storage, transcription, integrations, production Clerk verification, billing, analytics, automation and mobile code are not currently implemented.
