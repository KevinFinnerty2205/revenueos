...
7. Keep **Review before update** unless HubSpot must be the source of truth; that
   alternative blocks Oryntela writes.
8. Map only the Oryntela stages used by the team to exact HubSpot pipeline stages.
...
Contact link; Oryntela will not guess, merge or silently create one.
...
If HubSpot changed after preview, Oryntela asks for a fresh review. If an outcome
