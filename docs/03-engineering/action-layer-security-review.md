...
- no provider request or external execution path;
...
can still be inappropriate. Human review, clear source context and non-execution are
...
Connector credentials, delegated scopes, webhooks, delivery receipts, outbound
content scanning, autonomous execution, undo across external systems and production
execution incident response are not implemented.
