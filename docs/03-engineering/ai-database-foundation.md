...
WO-004A1 originally added persistence only. WO-004C1/C1A/C2/C3 now reuse it
for Executive Summary, Decisions and Action Items generated by the configured
...
- the allowed `infrastructure_test`, `executive_summary`, `decisions` or `action_items` job type and lifecycle state;
...
- Only infrastructure-test, Executive Summary, Decisions and Action Items
  behaviour exists.
- OpenAI is optional; no later Meeting Intelligence capability exists.
