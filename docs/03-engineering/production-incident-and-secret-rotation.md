...
| Stripe API/webhook secrets | disable new billing mutations; rotate test/live separately; deploy and verify mode-matched signatures plus read-only reconciliation; settle unknown operations before revoking old API authority | live adapter engineering exists but no live secret/account is configured; external activation remains blocked |
...
| Backup encryption key | create a new 32-byte base64 key; expose it only to the scheduled/restore jobs; retain old key in controlled recovery escrow for every unexpired backup; use new key for new archives; test format-v2 manifest and payload restoration; destroy old key only after its last archive expires | losing the old key makes retained payloads and authenticated manifests unusable; keep a separately controlled offline recovery copy; reuse across environments is prohibited |
