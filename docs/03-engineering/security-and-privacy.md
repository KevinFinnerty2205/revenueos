...
## Create PPTX and download hardening (WO-039B)

PPTX validation now rejects ambiguous ZIP paths/envelopes, unsafe compression,
unbounded XML/relationships, external or active content and signature-mismatched
media before `python-pptx`. Generated bytes are reparsed against exact expected
slides, replacements, claim text, required/legal content, Business Case values and
internal-data exclusions. Download credentials are random, SHA-256-only at rest,
absent from URLs/logs, short-lived, one-time and tenant/user/version/approval bound;
the authenticated API rechecks active membership and private-object checksum at
consume time. `create_download_grants` is tenant-owned with forced RLS. See the
[Create security review](create-security-privacy-review.md).
