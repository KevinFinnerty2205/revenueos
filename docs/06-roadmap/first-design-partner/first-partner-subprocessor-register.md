...
Zoho Mail has been selected and configured as the support mailbox provider. Its
operational selection and purchase do not complete legal/privacy or partner approval
for real partner data. Ordinary tickets must remain content-minimised.
...
| Gmail / Google Workspace API | Customer-connected reviewed Gmail sending, relevant reply reconciliation and bounded primary-calendar context | Connected-user identity; approved outbound mail; strongly correlated reply content/metadata; basic Calendar event metadata; encrypted delegated credential material | Customer Google Workspace and Google's applicable service locations; exact contracting, DPA, subprocessor and cross-border position is not approved by this register | Customer Workspace licence/admin policy; owner provider/privacy/legal approval; Cloud project, verified domain/consent screen, restricted-scope verification/security assessment, production secret and smoke-test evidence | **PROPOSED CUSTOMER-CONNECTED / NOT ACTIVATED; ADAPTER ONLY; NO ACCOUNT, PROJECT, MAILBOX, DATA OR REQUEST** |
