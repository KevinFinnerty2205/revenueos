...
| PostgreSQL | Lightsail managed PostgreSQL 16, 2 GB encrypted Standard plan, same Sydney region, public mode off | TLS; separate migration/admin and non-`BYPASSRLS` runtime roles; migration head `0062_live_stripe_billing`; automated seven-day point-in-time recovery plus portable backups |
