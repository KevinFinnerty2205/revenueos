...
| 2. Export | Run `revenueos-beta-maintenance export --organisation-id <uuid> --request-id <uuid>`; download through the authorised flow; validate export version 34/tenant/expected sections, including safe Credit and Prospect-provider outcome history, and expiry/cross-tenant denial | Verified portable export, no credentials/leases/provider costs/payloads/other tenant IDs; restricted temporary file handled under policy |
...
| 7. Delete organisation | Execute `revenueos-beta-maintenance delete-organisation --organisation-id <uuid> --request-id <uuid>` only after exact UI confirmation. If Billing or Credit transaction history exists, stop for the approved accounting-retention treatment | Command either completes once and is retry-safe, or fails closed on retained commercial history without deleting it |
