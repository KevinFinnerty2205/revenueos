...
| 2. Export | Run `revenueos-beta-maintenance export --organisation-id <uuid> --request-id <uuid>`; download through the authorised flow; validate export version 33/tenant/expected sections, including safe Credit history, and expiry/cross-tenant denial | Verified portable export, no credentials/leases/provider costs/payloads/other tenant IDs; restricted temporary file handled under policy |
