...
3. capture the best authorised evidence, including when recording is unavailable;
4. structure what happened and what changed;
5. review and correct the understanding;
6. prepare follow-up, record changes and next actions;
7. apply consequential work only through visible human control;
8. remember the outcome across the relationship; and
9. retrieve, explain and learn from that memory later.

The loop, not any individual screen or model, is the competitive floor.

## Core capability contract

| Customer job              | Minimum lovable Core capability                                                                       | Current state after WO-025                                   | Maturity requirement                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| Know what to do now       | Personal Daily with one explained priority                                                            | Implemented                                                  | Preserve simplicity; later add real target/forecast context                    |
| Prepare                   | Concise source-aware brief, objectives and important questions                                        | Implemented in bounded form                                  | Use current methodology/Brain context and connected schedule where approved    |
| Capture                   | Deliberate face-to-face, online, phone and no-recording paths                                         | Implemented in bounded/manual forms                          | Support at least one production-approved path; add providers by validated need |
| Transcribe                | Versioned transcript with partial/failure and speaker uncertainty                                     | Implemented architecture and optional adapter                | Operationally approve and evaluate the offered production path                 |
| Understand a conversation | Summary, signals, objections, competitors, people, decisions, actions, risks, questions and follow-up | Implemented                                                  | Reconcile all final sources into a concise deal narrative                      |
...
| Retrieve knowledge        | Search and evidence-backed questions                                                                  | Missing; Assistant is a placeholder                          | WO-025B before Prospect                                                        |
...
| Compare with goals        | Supported targets and KPI progress                                                                    | Missing                                                      | WO-037                                                                         |
| Forecast                  | Range, drivers, calibration and overrides                                                             | Missing                                                      | WO-038 after enough lifecycle/history evidence                                 |
| Coach and manage          | Fair opportunity-specific coaching and team exceptions                                                | Missing                                                      | WO-039 after analytics/forecast and permissions                                |
| Govern trust              | Tenant isolation, consent, retention, export, deletion and safe external processing                   | Strong implementation foundation; launch approval incomplete | Close target-environment gates before real design-partner use                  |

## What Core owns

Core retains its five product parts:
...
- **production CRM connection sufficient to read/match context and execute approved
  Core updates** because an external CRM may remain authoritative; and
- **the selected productivity connection needed by the offered Core workflow**, while
  provider breadth and advanced governance can be packaged only after the basic loop
  is complete.

## What remains an add-on
...
be evaluated later; a competitor's autonomous behaviour is not authority to remove
control.

## Minimum product claims

### Credible with the implemented private/synthetic boundary

- See what matters today and what to do next.
- Prepare from source-aware relationship memory.
...
Avoid absolute promises such as “know exactly”, “never write notes again”, “every
call” or “your CRM updates itself” unless the offered capture, reliability, consent,
provider and cohort evidence genuinely supports them.
