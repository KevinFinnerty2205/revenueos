# Design principles

RevenueOS should feel calm, capable and trustworthy.

## Qualities

- **Clean:** remove decorative noise and keep each screen focused on one job.
- **Fast:** prioritise quick navigation, immediate feedback and lightweight interfaces.
- **Calm:** use restrained colour, stable layouts and plain recovery messages.
- **Minimal:** show what is useful now; do not expose controls for unfinished features.
- **High trust:** label mocks, uncertainty, data use and unavailable integrations honestly.
- **Generous spacing:** separate concepts and preserve readable line lengths.
- **Clear hierarchy:** each page has one primary heading and an obvious reading order.
- **Accessible:** semantic HTML, keyboard navigation, visible focus, labelled controls, sufficient contrast and reduced-motion support are default requirements.
- **Low cognitive load:** use familiar terms, short sentences and purposeful empty states.

## Sprint 1 application rules

- Desktop-first layouts remain usable on narrow screens.
- Navigation remains reachable by keyboard and does not depend on hover.
- Empty states explain why content is absent and what future phase owns it.
- Do not invent customer names, metrics, activity or AI output.
- Do not use fake action buttons. A visible action must work.
- Animation is unnecessary unless it communicates loading; respect reduced-motion preferences.
